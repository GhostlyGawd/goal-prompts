...
`commands.zip`, `sitemap.xml`, `robots.txt`, `sw.js`.
...
(The `b/` and `p/` pages share one design system — `SITE_CSS` and the
`brief_detail`/`playbook_detail` builders in `build.py`. The landing page's
CSS lives in `template.html`; keep its tokens in step with `SITE_CSS`.)
