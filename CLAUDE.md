...
- `examples/index.html` — the hand-authored report gallery; it links into the
  generated `r/<slug>` styled pages (which render `reports/` + `examples/venture/`),
  so it moves in step when report files do
...
detail pages), `r/` (per-report styled pages — one `r/<slug>.html` per
`reports/*.md` and per `examples/venture/*.md`, rendered by build.py's
`render_report_md`/`report_page` so the proof surface isn't a raw-.md text
dump; the `.md` files stay put as each page's raw escape hatch), `plugin/`
(the Claude Code plugin — commands and
