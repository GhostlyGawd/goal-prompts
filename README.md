...
`studio.html`, `vitals.html`, `coverage.html`, `build.py`, `install`, `mcp/`,
`js/` (the site's tested modules — catalog core, report parser, detail-page
helpers), `scripts/`, `tests/`, `docs/`, `.claude-plugin/marketplace.json`.
Generated by the build (never hand-edit): `index.html`, `raw/`, `b/`, `p/`,
`catalog.json`, `bodies.json`, `coverage.json`, `checksums.txt`, `commands.tar.gz`,
