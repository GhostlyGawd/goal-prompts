...
> excerpt near the finder; post-win momentum) — **A1 is now FIXED** (the copy→paste next-step hint shipped, naming the brief's output file); **A2 is now FIXED** (a "New here? Start with the Day-1 playbook →" starter leads the finder); **A5** (post-win momentum) — **FIXED**: the copy hint tees up step 2 with a Report Studio link. **A4** (real report excerpt near the finder) — **FIXED**: a "See a real report one wrote →" link sits by the finder. **A3** (hero quickstart) — **DEFERRED** (the report scopes it as an A/B test). These are UX/product
