# Playbook: All Subtract briefs (conductor)

You are working inside this repo. Mission: execute the **All Subtract briefs** playbook — 2 audit briefs in sequence, each producing one report file at the repo root (or in `reports/`, if the repo has that directory).

Every Subtract brief in the catalog, in order — 26 through 27, one report each.
