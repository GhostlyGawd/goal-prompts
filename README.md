...
carries a severity and a confidence, and ends in a fix sketch. Then the
**Act** family closes the loop: report → [Studio](https://goal-prompts.vercel.app/studio)
→ Fixer → [FIXLOG](https://goal-prompts.vercel.app/r/fixlog) — audits that
end in commits, not chat scrollback.

Sixty seconds to a first report:

```
/plugin marketplace add GhostlyGawd/goal-prompts
/plugin install goal@goal-prompts
```

then, inside any repo you want audited: `/goal:bug-hunt` — and read the
`BUGS.md` it files. Every brief is read-only (its only write is its report),
ends by asking before anything changes, stays under 4,000 characters, and
passes a machine-enforced quality bar or the build fails.

Proof, not promises: [the reports this catalog ran against its own repo](https://goal-prompts.vercel.app/examples/),
committed unedited, with the FIXLOG tracing findings into shipped commits.
...
at **`/p/<key>`** with a step-by-step sequence map, and every dogfood report
renders readable at **`/r/<slug>`** (the raw `.md` stays one click away). Playbooks can also be
...
Want just one brief as a persistent command? `BRIEF` picks it by id or slug
and leaves any other installed briefs untouched:

```
curl -fsSL https://goal-prompts.vercel.app/install | BRIEF=01 sh
```

Two more native packagings ship from the same build:

- **Cursor** — `cursor-commands.zip` unzips at your repo root into
  `.cursor/commands/goal-<slug>.md`, Cursor's project-commands format, so
  every brief is a `/goal-<slug>` command there too.
- **Skills** — every brief also exists as `skills/goal-<slug>/SKILL.md`
  (the `SKILL.md` packaging with `name`/`description` frontmatter); copy any
  skill directory into `.claude/skills/` to use it.
...
Or let your agent draft it: the **Brief Forge**
([docs/brief-forge.md](docs/brief-forge.md)) is a copyable meta-prompt that
writes the brief and iterates against `build.py`'s linter until it passes.
...
`r/` (the rendered report pages), `catalog.json`, `bodies.json`,
`checksums.txt`, `commands.tar.gz`,
`commands.zip`, `cursor-commands.zip`, `plugin/`, `skills/`, `quality.html`,
`changelog.html`, `sitemap.xml`, `robots.txt`, `sw.js`, `tokens.css`.
