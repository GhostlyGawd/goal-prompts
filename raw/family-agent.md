...
You are working inside this repo. Mission: execute the **All Agent briefs** playbook — 12 audit briefs in sequence, each producing one report file at the repo root (or in `reports/`, if the repo has that directory).

Every Agent brief in the catalog, in order — 30 through 50, one report each.
