# Playbook: Stay on Mission (conductor)

You are working inside this repo. Mission: execute the **Stay on Mission** playbook — 3 audit briefs in sequence, each producing one report file at the repo root (or in `reports/`, if the repo has that directory).

Stop the drift loop: write the one-page charter this repo has been missing, hold every shipped surface against it, then let the Fixer delete what nobody asked for — and leave with a contract every future session reads before touching product code.
...
## Stages
1. **149 · The Charter** — fetch https://goal-prompts.vercel.app/raw/149.md → writes `CHARTER.md`
2. **150 · Drift Audit** — fetch https://goal-prompts.vercel.app/raw/150.md → writes `DRIFT.md`
