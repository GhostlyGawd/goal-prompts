...
`commands.tar.gz`, `commands.zip`, `sitemap.xml`, `robots.txt`, `sw.js`.
...
- Family order and questions have one source: `FAMILY_ORDER` in `build.py`
  ...
  `__FAMILIES_JSON__`. Family *colors* still live in two places —
  `template.html`'s `.f-*` CSS rules and `scripts/og.py` — add both when
  adding a family.
