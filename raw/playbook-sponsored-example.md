# Playbook: Sponsored Speed Bundle (conductor)

You are working inside this repo. Mission: execute the **Sponsored Speed Bundle** playbook — 3 audit briefs in sequence, each producing one report file at the repo root (or in `reports/`, if the repo has that directory).

A worked example of a sponsored playbook: a curated performance sequence a sponsor's brand wraps — logo, name, and voice around shared content, with the placement paid. Swap in a real sponsor to ship one.
...
3. **88 · Bundle & Asset Weight Audit** — fetch https://goal-prompts.vercel.app/raw/88.md → writes `BUNDLE.md`
